...
Immich requires [Docker](/reference/docker) with the Docker Compose plugin. Immich requires the `docker compose` command; the similarly named `docker-compose` is [deprecated](https://docs.docker.com/retired/#docker-compose-v1-replaced-by-compose-v2) and is no longer supported.
